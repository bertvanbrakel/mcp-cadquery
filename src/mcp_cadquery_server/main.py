...
async def push_sse_message(message_data: dict):
    ...
    if not message_data: return # Don't push empty messages
...
async def _process_and_push(request: dict):
    ...
    message_to_push = process_tool_request(request) # Now returns the message dict
...
async def execute_tool_endpoint(request_body: dict = Body(...)):
    ...
    asyncio.create_task(_process_and_push(request_body)) # Use the wrapper
...
def process_tool_request(request: dict) -> Optional[dict]:
    ...
    message_to_push: Optional[dict] = None # Default to None
    if error_message:
        ...
        # This case might happen if a handler intentionally returns None or has no return
        log.warning(f"No result or error message generated for request ID: {request_id}")
        # Optionally send a generic error, or just return None
        # message_to_push = {"type": "tool_error", "request_id": request_id, "error": "Handler produced no result or error."}

    # Removed await push_sse_message(message_to_push)
    return message_to_push # Return the dictionary

# --- Tool Implementations ---
# (Unchanged - they remain synchronous)
...
def handle_scan_part_library(request) -> dict:
        ...
        if filename.endswith(".py") and not filename.startswith("_"):
            ...
            try:
                ...
                if cached_data and cached_data.get('mtime') == current_mtime:
                    log.debug(f"Using cached data for part: {filename}"); cached_count += 1; continue
    ...
    for part_name_to_remove in parts_to_remove:
        ...
        if removed_data and 'preview_url' in removed_data:
            ...
            preview_file_path = os.path.join(PART_PREVIEW_DIR_PATH, preview_filename)
...
def handle_search_parts(request) -> dict:
    ...
    try:
        ...
        error_msg = f"Error during part search: {e}"; log.error(error_msg, exc_info=True); raise Exception(error_msg)
...
async def run_stdio_mode():
    """Runs the server in MCP stdio mode, reading/writing JSON lines."""
    log.info("Starting server in MCP stdio mode...")
    # Use asyncio to read stdin line by line
    ...
    while True:
        try:
            ...
            if not line_bytes: # EOF
                log.info("Stdin closed, exiting stdio mode.")
                break
            ...
            try:
                ...
                if response_data:
                    ...
                    log.debug(f"Sending stdio response: {response_json}")
                    print(response_json, flush=True) # Write JSON line to stdout and flush

            except json.JSONDecodeError as e:
                log.error(f"Failed to decode JSON from stdin: {e}")
                # Send error back if possible
                error_resp = {"type": "tool_error", "request_id": "unknown", "error": f"Invalid JSON received: {e}"}
                print(json.dumps(error_resp), flush=True)
            except Exception as e:
                 log.error(f"Error processing stdio request: {e}", exc_info=True)
                 # Try to extract request_id for error reporting
                 req_id = request_data.get("request_id", "unknown") if isinstance(request_data, dict) else "unknown"
                 error_resp = {"type": "tool_error", "request_id": req_id, "error": f"Internal server error: {e}"}
                 print(json.dumps(error_resp), flush=True)

        except KeyboardInterrupt:
            log.info("KeyboardInterrupt received, exiting stdio mode.")
            break
        except Exception as e:
            log.error(f"Unexpected error in stdio loop: {e}", exc_info=True)
...
def main(
    ...
    stdio: bool = typer.Option(False, "--stdio", help="Run in MCP Stdio mode."), # Keep flag
    library_dir: Optional[str] = typer.Option(None, "--library-dir", "-l", help="Path to the CadQuery part library directory (overrides default relative to package)."),
    static_dir: Optional[str] = typer.Option(None, "--static-dir", "-s", help="Path to the static files directory (overrides default relative to package)."),
    ...
    if stdio:
        # Configure logging for stdio (maybe simpler format?)
        # logging.basicConfig(level=logging.INFO, format='%(levelname)s: %(message)s', stream=sys.stderr) # Example
        asyncio.run(run_stdio_mode())
        raise typer.Exit() # Exit after stdio mode finishes

    # --- HTTP Mode Execution ---
    # Determine default paths relative to package location if not overridden
    if library_dir is None:
        try:
            with pkg_resources.path(__package__, PART_LIBRARY_DIR) as p: PART_LIBRARY_DIR = str(p)
        except FileNotFoundError: log.warning(f"Default part library '{PART_LIBRARY_DIR}' not found. Using relative path."); PART_LIBRARY_DIR = os.path.abspath(PART_LIBRARY_DIR)
    else: PART_LIBRARY_DIR = os.path.abspath(library_dir)

    if static_dir is None:
        try:
            with pkg_resources.path(__package__, os.path.join("frontend", "dist")) as p: STATIC_DIR = str(p)
        except FileNotFoundError: log.warning("Default static dir 'frontend/dist' not found. Using relative path."); STATIC_DIR = os.path.abspath(os.path.join("frontend", "dist"))
    else: STATIC_DIR = os.path.abspath(static_dir)

    PART_PREVIEW_DIR_NAME = preview_dir_name; RENDER_DIR_NAME = render_dir_name
    RENDER_DIR_PATH = os.path.join(STATIC_DIR, RENDER_DIR_NAME); PART_PREVIEW_DIR_PATH = os.path.join(STATIC_DIR, PART_PREVIEW_DIR_NAME)
    ASSETS_DIR_PATH = os.path.join(STATIC_DIR, "assets")
    ...
    log.info(f"Starting Uvicorn server on {host}:{port}..."); uvicorn.run("mcp_cadquery_server.main:app", host=host, port=port, reload=reload, app_dir="src")

# Removed check_and_setup_env()

if __name__ == "__main__":
    cli()                    shape_to_preview = build_result.results[0].shape
                    preview_filename = f"{part_name}.svg"
                    preview_output_path = os.path.join(preview_dir_path, preview_filename)
                    preview_output_url = f"{preview_dir_url}/{preview_filename}"
                    export_shape_to_svg_file(shape_to_preview, preview_output_path, default_svg_opts)
